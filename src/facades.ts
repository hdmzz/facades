...
//on agit sur une cellule dans cette fonction
export function unflattenPolygon(
	...
	const unprojectedCell: ThreePoint[] = [];

	//map ((chose) => si pas de {} alors pas besoin de return mais une virgule )
	cell.forEach((ring) => {
		ring.forEach((point) => {
			unprojectedCell.push(unprojectPoint(u, v, origin, point));
		})
	});
	const indices = earcut(vertices, [], 2);

	return {
		point: unprojectedCell,
		indices: indices
	}
}
